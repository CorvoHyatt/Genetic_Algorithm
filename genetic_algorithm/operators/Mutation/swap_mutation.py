...
class MutationSwap(MutationOperator):
    @staticmethod
    def mutate(population: List[Individual], probability: int) -> List[Individual]:
        ...
        for individual in population:
            ...
            if random_prob <= probability:
                ...
                # Seleccionar dos índices aleatorios diferentes y realizar el intercambio de bits
                index1, index2 = np.random.choice(
                    len(mutated_genotype), 2, replace=False
                )
                mutated_genotype[index1], mutated_genotype[index2] = (
                    mutated_genotype[index2],
                    mutated_genotype[index1],
                )
